...
const Element = ({icon, text, selected}) => {
    if (selected === true) {
    return (
        <div className='flex w-max font-bold text-xl font-sans hover:bg-slate-200 rounded-full h-[3.75rem] items-center transition-all'>
            <div className='m-3'>{icon}</div>
            <div className='m-3'>{text}</div>
        </div>
    )}
    return (
        <div className='flex w-max text-xl font-sans hover:bg-slate-200 rounded-full h-[3.75rem] items-center transition-all'>
            <div className='m-3'>{icon}</div>
            <div className='m-3'>{text}</div>
        </div>
    )
...
const LeftSidebar = () => {
    return (
        <div className="h-screen pr-6 w-72 border-r border-slate-200 flex flex-col justify-between">
